		...
		*/
		...
		{	/*
			...
			{	$youtube_id = substr($matches[2][0], 0, 35);
				//iframe 형태로 출력시 ID 값이 먼저 오기 때문에 '&amp;'를 '?'로 치환
				//$youtube_id = str_replace('&amp;', '?', $youtube_id).'&amp;';
			}
			else
...
* * * * * */
			...
			{	// Youtube iframe 스타일
				...
				return '<div class="trailer"><div id="player'.(string)$yt_counter.'"></div></div>'.$yt_html_code;
			}
			else
		...
		*/
		...
		{	$naver_id = $match[1];
			...
			return sprintf('<div class="movie_info"><iframe src="//movie.naver.com/movie/bi/mi/basic.nhn?code=%s" scrolling="no" frameborder="0"></iframe></div>', $naver_id);
		}
